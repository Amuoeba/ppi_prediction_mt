...
class PDBindDataset:
    ...
    def __init__(self, root):
        ...
        if self.check_db_exist():
            ...
            conn = sqlite3.connect(config.folder_structure_cfg.PDBind_sql)
    ...
    def check_db_exist(self) -> bool:
        ...
        """
        if os.path.exists(config.folder_structure_cfg.PDBind_sql):
            return True
        else:
            return False
...
if __name__ == '__main__':
    ...
    # pdbind_dataset = PDBindDataset(config_old.PDBIND_DATA_ROOT)
    #
    ...
    pdbind_dataset = PDBindDataset(config.folder_structure_cfg.PDBind_data)