...
def update_activations_image(slider_value, selected_experiment, selected_sample, selected_layer):
    ...
    for image in os.listdir(
            f"{config.folder_structure_cfg.log_path}/{selected_experiment}/nn_vis/{slider_value}/{selected_sample}/{selected_layer}/"):
...
@app.callback(
    [Output("train-graph", "children"), Output("val-graph", "children"),
     Output("train-accuracy-graph", "children"), Output("val-accuracy-graph", "children")],
    [Input('experiment-dropdown', 'value'),
     Input('interval-component', 'n_intervals'),
     Input('moving-average-slider', "value")]
)
def update_train_and_val_graphs(selected_experiment, update_interval, moving_average):
    print(
    ...
    train_fig = train_val_figure(train_log_path, "loss", int(moving_average), color="#3248a8", title="Train loss",
                                 xaxis_title="batch", yaxis_title="Loss")
    val_fig = train_val_figure(val_log_path, "loss", int(moving_average), color="#3248a8", title="Validation loss",
                               xaxis_title="batch", yaxis_title="Loss")
    train_accuracy_fig = train_val_figure(train_log_path, "batch_acc", int(moving_average), color="#a86332",
                                          title="Train accuracy", xaxis_title="batch", yaxis_title="Accuracy")
    val_accuracy_fig = train_val_figure(val_log_path, "batch_acc", int(moving_average), color="#a86332",
                                        title="Validation accuracy", xaxis_title="batch", yaxis_title="Accuracy")

    return dcc.Graph(figure=train_fig), dcc.Graph(figure=val_fig), \
           dcc.Graph(figure=train_accuracy_fig), dcc.Graph(figure=val_accuracy_fig)
